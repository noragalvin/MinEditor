function init(id){content=`<style>
.k-caret {
...
<div id="cal1">&nbsp;</div>
<div id="cal2">&nbsp;</div>
`;var editor=document.getElementById(id);editor.innerHTML+=content;function setEndOfContenteditable(contentEditableElement){var range,selection;if(document.createRange)
{range=document.createRange();range.selectNodeContents(contentEditableElement);range.collapse(!1);selection=window.getSelection();selection.removeAllRanges();selection.addRange(range)}else if(document.selection)
{range=document.body.createTextRange();range.moveToElementText(contentEditableElement);range.collapse(!1);range.select()}}
window.formatDoc=function(sCmd,sValue){if(sCmd=='h1'||sCmd=='h2'||sCmd=='h3'||sCmd=='h4'||sCmd=='h5'||sCmd=='h6'||sCmd=='p'){document.execCommand('formatBlock',!1,sCmd)}else if(sCmd=='createLink'){setEndOfContenteditable(elem);var data=document.getElementById('data-link').value;document.execCommand(sCmd,!0,data);document.getElementById.value="";$('#kee-tool').css('dislay','none')}else if(sCmd==='backcolor'||sCmd==='forecolor'){document.execCommand(sCmd,!0,sValue)}else{if(document.queryCommandSupported(sCmd)==!1){alert('The command '+sCmd+' is not support your browser')}else{$('.dropdown-font-family').css('display','none');$('.dropdown-paragraph-format').css('display','none');$('.dropdown-color').css('display','none');$('.dropdown-font-size').css('display','none');$('.dropdown-align').css('display','none');document.execCommand(sCmd,!1,sValue)}}
selectingText()}
window.insertImg=function(img){elem=document.getElementById('editor-doc');setEndOfContenteditable(elem);var url=document.getElementById('data-img').value;var inputUrl=url;url="<img class=\"child\" src=\""+inputUrl+"\">";console.log("url:"+inputUrl);if(inputUrl){document.execCommand("insertHTML",!1,url);document.getElementById('data-img').value=""}}
window.insertVideo=function(){elem=document.getElementById('editor-doc');setEndOfContenteditable(elem);sVideo=document.getElementById('data-video').value;var inputVideoURL=sVideo;console.log("url: "+inputVideoURL);var check=sVideo.substr(1,6);if(inputVideoURL){if(check==="iframe"){document.execCommand("insertHTML",!1,sVideo);document.getElementById('data-video').value=""}else{var idVideo=sVideo.substr(32,sVideo.length-1);stringVideo="https://www.youtube.com/embed/"+idVideo;var widthVideo=document.getElementById('editor-doc').offsetWidth;var heightVideo=0.5625*widthVideo;sVideo="<iframe width="+widthVideo+"\" height="+heightVideo+" src=\""+stringVideo+"\" frameborder=\"0\" webkitallowfullscreen=\"\" mozallowfullscreen=\"\" allowfullscreen=\"\" __idm_id__=\"189403137\"></iframe>"
document.execCommand("insertHTML",!1,sVideo);document.getElementById('data-video').value=""}}}
$(document).ready(function(){$(window).keyup(function(e){var data=$('.embed-container').html();if(data==='<br>'){console.log(2);$(this).remove()}})});window.onload=function(){var ed=document.querySelector("#editor-doc");ed.addEventListener("paste",function(e){e.preventDefault();var text=e.clipboardData.getData("text/plain");document.execCommand("insertHTML",!1,text)})}
$(document).ready(function(){var flag=1;var data=document.getElementById('editor-doc');$('#showCode').click(function(e){e.preventDefault();if(flag===1){document.getElementById('editor-doc').innerText=data.innerHTML.trim();flag=0}else{document.getElementById('editor-doc').innerHTML=data.innerText.trim();flag=1}})});$(document).ready(function(){function getSelected(){if(window.getSelection){console.log('selected');return window.getSelection()}else if(document.getSelection){console.log('selected');return document.getSelection()}else{var selection=document.selection&&document.selection.createRange();if(selection.text){console.log('selected');return selection.text}
return!1}
return!1}});var ele=document.getElementById('kee-tool');var sel=window.getSelection();var rel1=document.createRange();rel1.selectNode(document.getElementById('cal1'));var rel2=document.createRange();rel2.selectNode(document.getElementById('cal2'));document.getElementById('editor-doc').addEventListener('mouseup',function(event){event.stopPropagation();if(!sel.isCollapsed){var r=sel.getRangeAt(0).getBoundingClientRect();var rb1=rel1.getBoundingClientRect();var rb2=rel2.getBoundingClientRect();ele.style.top=(r.bottom-rb2.top)*100/(rb1.top-rb2.top)+'px';ele.style.left=((r.left-rb2.left+10)*100/(rb1.left-rb2.left))+'px';ele.style.display='block'}});$(document).ready(function(){$(window).resize(function(){ele.style.display='none'})});$('body').mousedown(function(){$('#kee-tool').hover(function(){},function(){$('.dropdown-font-family').hide();$('.dropdown-paragraph-format').hide();$('.dropdown-color').hide();$('.dropdown-font-size').hide();$('.dropdown-align').hide();ele.style.display='none'})});function selectingText(){if(!sel.isCollapsed){var r=sel.getRangeAt(0).getBoundingClientRect();var rb1=rel1.getBoundingClientRect();var rb2=rel2.getBoundingClientRect();ele.style.top=(r.bottom-rb2.top)*100/(rb1.top-rb2.top)+'px';ele.style.left=((r.left-rb2.left+10)*100/(rb1.left-rb2.left))+'px';ele.style.display='block'}}
$('body').mousedown(function(e){if(e.which==1){}});$.fn.setCursorPosition=function(pos){this.each(function(index,elem){if(elem.setSelectionRange){elem.setSelectionRange(pos,pos)}else if(elem.createTextRange){var range=elem.createTextRange();range.collapse(!0);range.moveEnd('character',pos);range.moveStart('character',pos);range.select()}});return this};$(document).ready(function(){$("#sub-tools").hide();$("#talign").hide()});window.showmore=function(event){$('.dropdown-font-family').css('display','none');$('.dropdown-paragraph-format').css('display','none');$('.dropdown-color').css('display','none');$('.dropdown-font-size').css('display','none');$('.dropdown-align').css('display','none');$("#sub-tools").show();$("#show").hide();$("#talign").show()};window.showoff=function(event){$('.dropdown-font-family').css('display','none');$('.dropdown-paragraph-format').css('display','none');$('.dropdown-color').css('display','none');$('.dropdown-font-size').css('display','none');$('.dropdown-align').css('display','none');$("#sub-tools").hide();$("#show").show();$("#talign").hide()};$(window).on('load',function(){var elem=document.getElementById('editor-doc');setEndOfContenteditable(elem)});$('#editor-doc').keydown(function(e){var elem=document.getElementById('editor-doc');var data=document.getElementById('editor-doc').innerText;if(data==="\n"){setEndOfContenteditable(elem);console.log(1);$('#editor-doc').hover(function(){setEndOfContenteditable(elem)})}});function setCaret(line,col){var ele=document.getElementById("editable");var rng=document.createRange();var sel=window.getSelection();rng.setStart(ele.childNodes[line],col);rng.collapse(!0);sel.removeAllRanges();sel.addRange(range);ele.focus()}
function insertTab(){if(!window.getSelection)return;const sel=window.getSelection();if(!sel.rangeCount)return;const range=sel.getRangeAt(0);range.collapse(!0);const span=document.createElement('span');span.appendChild(document.createTextNode('\t'));span.style.whiteSpace='pre';range.insertNode(span);range.setStartAfter(span);range.collapse(!0);sel.removeAllRanges();sel.addRange(range)}
$(document).on('keydown','#editor-doc',function(e){if(e.keyCode==9){insertTab();e.preventDefault()}});$('#text-color a').click(function(e){e.preventDefault();$('.dropdown-color').hide()});var modalImg=document.getElementById('myModalImg');var btnImg=document.getElementById("myBtnImg");btnImg.onclick=function(){modalImg.style.display="block"}
var modalVid=document.getElementById('myModalVid');var btnVid=document.getElementById("myBtnVid");btnVid.onclick=function(){modalVid.style.display="block"}
var modalLink=document.getElementById('myModalLink');var btnLnk=document.getElementById("myBtnLink");btnLnk.onclick=function(){modalLink.style.display="block"}
window.onclick=function(event){if(event.target==modalImg){modalImg.style.display="none"}
if(event.target==modalVid){modalVid.style.display="none"}
if(event.target==modalLink){modalLink.style.display="none"}}
$(document).keyup(function(e){if(e.keyCode==27){modalVid.style.display="none";modalImg.style.display="none";modalLink.style.display="none"}});$('.k-close-modal').click(function(e){e.preventDefault();modalVid.style.display="none";modalImg.style.display="none";modalLink.style.display="none"});$('.k-button-green').click(function(e){e.preventDefault();modalVid.style.display="none";modalImg.style.display="none";modalLink.style.display="none"});window.backgroundColor=function(){$('#text-color').css('display','none');$('#bg-color').fadeIn();$('#background').css({'border':'none','border-top':'0.7px solid gray','border-right':'0.7px solid gray','border-left':'0.7px solid gray','color':'black'});$('#text').css({'border':'none','border-bottom':'0.7px solid gray','color':'gray'})}
window.textColor=function(){$('#text-color').fadeIn();$('#bg-color').css('display','none');$('#text').css({'border':'none','border-top':'0.7px solid gray','border-right':'0.7px solid gray','border-left':'0.7px solid gray','color':'black'});$('#background').css({'border':'none','border-bottom':'0.7px solid gray','color':'gray'})}
window.buttonColor=function(){$('#font-family').css('display','none');$('.paragraph-format').css('display','none')}
$('#button-font-family').click(function(){$('.dropdown-font-family').toggle();$('.dropdown-paragraph-format').hide();$('.dropdown-color').hide();$('.dropdown-font-size').hide();$('.dropdown-align').hide()})
$('#button-paragraph-format').click(function(){$('.dropdown-paragraph-format').toggle();$('.dropdown-font-family').hide();$('.dropdown-color').hide();$('.dropdown-font-size').hide();$('.dropdown-align').hide()})
$('#button-color').click(function(){$('.dropdown-color').toggle();$('.dropdown-font-family').hide();$('.dropdown-paragraph-format').hide();$('.dropdown-font-size').hide();$('.dropdown-align').hide()})
$('#button-font-size').click(function(){$('.dropdown-font-size').toggle();$('.dropdown-font-family').hide();$('.dropdown-paragraph-format').hide();$('.dropdown-color').hide();$('.dropdown-align').hide()})
$('#button-align').click(function(){$('.dropdown-align').toggle();$('.dropdown-font-family').hide();$('.dropdown-paragraph-format').hide();$('.dropdown-color').hide();$('.dropdown-font-size').hide()})
window.buttonBackground=function(){if($('.dropdown-font-family').css('display')=='block'){$('#button-font-family').css('background-color','#dce0e8')}else{$('#button-font-family').css('background-color','transparent')}
...
var editable=document.getElementById('editor-doc');editable.addEventListener('input',function(){var data=document.getElementById('editor-doc').innerHTML;if(data==""){document.getElementById('editor-doc').innerHTML="<p><br/></p>"}});window.uploadImg=function(){var formData=new FormData();var file=document.getElementById('myFile').files[0];console.log("x",file);formData.append('image',file);console.log(formData);$.ajax({type:'POST',url:"upload.php",data:formData,cache:!1,contentType:!1,processData:!1,dataType:"text",xhr:function(){$('#percent').show();var xhr=new window.XMLHttpRequest();xhr.upload.addEventListener("progress",function(evt){if(evt.lengthComputable){var percentComplete=evt.loaded/evt.total;percentComplete=parseInt(percentComplete*100);$('.myprogress').text(percentComplete+'%');$('.myprogress').css('width',percentComplete+'%')}},!1);return xhr},success:function(data){$('#percent').hide();modalVid.style.display="none";modalImg.style.display="none";modalLink.style.display="none";console.log("success");var i=document.getElementById("editor-doc");elem=document.getElementById('editor-doc');setEndOfContenteditable(elem);var url="<div><img src=\""+data+"\" width=100% height=auto></div>";document.execCommand('insertHTML',!1,url.trim())},error:function(data){console.log("error");console.log(data)}})}
var savedRange,isInFocus;window.saveSelection=function(){if(window.getSelection)
{savedRange=window.getSelection().getRangeAt(0)}else if(document.selection)
{savedRange=document.selection.createRange()}}
window.restoreSelection=function(){isInFocus=!0;document.getElementById("editor-doc").focus();if(savedRange!=null){if(window.getSelection)
{var s=window.getSelection();if(s.rangeCount>0)
s.removeAllRanges();s.addRange(savedRange)}else if(document.createRange)
{window.getSelection().addRange(savedRange)}else if(document.selection)
{savedRange.select()}}}
var isInFocus=!1;window.onDivBlur=function(){isInFocus=!1}
window.cancelEvent=function(e){if(isInFocus==!1&&savedRange!=null){if(e&&e.preventDefault){e.stopPropagation();e.preventDefault()}else{window.event.cancelBubble=!0}
restoreSelection();return!1}}
$(".child").resizable({aspectRatio:!0,minWidth:100,maxWidth:$("#editor-doc").width(),containment:"editor-doc",handles:"ne,nw,se,sw",resize:function(event,ui){var topB=(parseInt($(this).css("top"))>0)?parseInt($(this).css("top")):3;var leftB=(parseInt($(this).css("left"))>0)?parseInt($(this).css("left")):3;if(parseInt($(this).css("left"))<3)
{$(this).trigger('mouseup');$(this).css({"left":leftB+"px","top":topB+"px"})}
if(parseInt($(this).css("top"))<3)
{$(this).trigger('mouseup');$(this).css({"left":leftB+"px","top":topB+"px"})}}}).draggable({containment:"editor-doc"})}